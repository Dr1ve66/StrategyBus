...
Критерии должны:
- относиться или к открытым данным, т.е. их можно найти в интернете (например:отраль, оквэд, регион,наличие товарного знака, лицензии) и/или к данным которые могут есть в банке (например:количество покупателей и поставщиков, 
количество сотрудников, размер выручки, срок деятельности, назначения платежей). Не предлагай Критерии которые можно узнать только работая в самой компании (например: есть ли в штате должность юриста, 
...
Пример: запуск интернет-магазина, участие в торгах, запуск рекламной кампании.
...
def register_routes(app):
    ...
    def process():
        ...
        if not input_text:
            flash("Введите данные для обработки.", "warning")
...
Критерии: {selected.final_criteria}"""
    ...
    def agent2_finish(selected_id):
        ...
        fixed_title = fix_mojibake(payload["title"])
        fixed_description = fix_mojibake(payload["description"])
        fixed_logic = fix_mojibake(payload["logic"])
        fixed_criteria = fix_mojibake(payload["criteria"])
