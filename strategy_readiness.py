...
BANK_CENTRIC_PATTERNS = (
    ...
    # банковская реактивация / удержание клиента для банка
    r"(?:нужно|надо)\s+(?:его|её|их)\s+реактивировать",
    r"не\s+пользуется\s+расчётным\s+счётом",
    r"(?:истекает|заканчивается|закрывается)\s+(?:депозит|вклад)",
    r"(?:нужно|надо)\s+удержать\s+(?:депозит|вклад)",
    # "почему клиенты не берут кредиты/продукты" — банковская аналитика
    r"почему\s+клиент\w*\s+не\s+берут?\s+(?:кредит|продукт|услуг)",
)
...
# Descriptions where the bank manager talks about their own task, not the client's business.
MANAGER_TASK_PATTERNS = (
    # "у меня есть список/портфель [клиентов]"
    r"у\s+меня\s+(?:есть|имеется)\s+\w*\s*(?:список|портфель|клиент)",
    # "мне нужно составить/подготовить [аналитику]"
    r"мне\s+нужно\s+(?:составить|подготовить)\b",
    # "нужно составить портрет/профиль/список [клиентов]"
    r"(?:нужно|надо)\s+составить\s+(?:портрет|профил|список)",
    # "нужно выполнить план [продаж]"
    r"(?:нужно|надо|хочу|хотим)\s+выполнить\s+план\b",
    # "нужно подготовить КП / коммерческое предложение / презентацию"
    r"(?:нужно|надо)\s+подготовить\s+(?:кп\b|коммерческ|презентаци)",
    # "нужна стратегия для работы с [клиентами/сегментом]" — банковская продажная стратегия
    r"нужна\s+стратегия\s+(?:для|по)\s+работ[ыи]\s+с\b",
    # "хочу предложить клиентам [что-либо]" — менеджер инициирует продажу
    r"(?:хочу|хотим)\s+предложить\s+клиент",
    # "нужно найти клиентов для [банковского продукта]"
    r"(?:нужно|надо)\s+найти\s+клиент\w+\s+для\s+(?:продукт|услуг|зарплатн|факторинг|кредит|лизинг|эквайринг)",
)

MANAGER_TASK_REWRITE_HINT = (
    "Опишите проблему или возможность бизнеса клиента, а не свою задачу. "
    "Например: «у клиента падают продажи», «клиент испытывает кассовый разрыв», "
    "«клиент хочет выйти на новый рынок»."
)


def is_manager_task(text: str) -> bool:
    """Detect descriptions where the bank manager describes their own task."""
    normalized = sanitize_situation_for_check(text).lower()
    if not normalized:
        return False
    for pattern in MANAGER_TASK_PATTERNS:
        if re.search(pattern, normalized, re.IGNORECASE):
            return True
    return False
...
def run_guards(text: str) -> dict | None:
    ...
    if is_manager_task(normalized):
        return _reject(
            reason="Описание похоже на задачу менеджера банка, а не на проблему бизнеса клиента.",
            rewrite_hint=MANAGER_TASK_REWRITE_HINT,
            missing=[CLARIFY_REWRITE],
            score=10,
        )
